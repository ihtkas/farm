...
func main() {
	...
	flag.StringVar(&pgUsername, "pg_uname", "postgres", "User to connect postgres server")
	flag.StringVar(&pgPassword, "pg_password", "postgres", "Password to connect postrgres server")
	flag.StringVar(&pgHost, "pg_host", "127.0.0.1", "Host address of postgres server")
	flag.StringVar(&pgPort, "pg_port", "5432", "Port in the host to connect postgres server")
	flag.StringVar(&pgDbname, "pg_dbname", "farm_seller", "Database name for seller service in postgres")
	flag.Parse()
